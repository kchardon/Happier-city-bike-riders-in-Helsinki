...
def get_time_codes():
    ...
    now += timedelta(minutes=5-((now.minute)%5))
    if now <= datetime.now().replace(microsecond=0):
...
def predict_balancing():
    ...
    for i in station_status['bikeRentalStations']:
        ...
        time, day_of_week, day_of_year, human_readable = times[0]
        if(round(bikes) <= 1):
                need_more_bikes.append((human_readable, i['name']))
                continue
        if(capacity == 0):
            capacity = round(bikes) + spaces
            if(round(bikes) >= capacity + 10):
                overflow = round(bikes)-capacity
                overflow_of_bikes.append((human_readable, i['name'], overflow))
                continue
        else:
            if(round(bikes) >= capacity + 10):
                overflow = round(bikes)-capacity
        ...
        for j in range(1,len(times)):
            ...
            bikes =  bikes + arrival_model.predict(data)[0] - departure_model.predict(data)[0]
            if(round(bikes) <= 1):
                need_more_bikes.append((human_readable, i['name']))
                break
            if(capacity == 0):
